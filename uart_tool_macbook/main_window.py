...
class MainWindow(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        file_menu.addAction(self._make_action("保存日志", self._receive_panel.save_log_dialog, "Ctrl+S"))
        file_menu.addSeparator()
        quit_act = QAction("退出", self)
        quit_act.triggered.connect(self.close)
        quit_act.setShortcut(QKeySequence.StandardKey.Quit)  # macOS 上自动为 ⌘Q
        file_menu.addAction(quit_act)

        view_menu = mb.addMenu("视图")
        focus_act = self._make_action("全屏日志模式", self._toggle_log_focus)
        # macOS 标准全屏快捷键 ⌃⌘F；同时保留 F11 作为备选触发
        focus_act.setShortcuts([
            QKeySequence(QKeySequence.StandardKey.FullScreen),
        ...
        view_menu.addAction(self._make_action("清空接收区", self._receive_panel.clear, "Ctrl+L"))
        view_menu.addAction(self._make_action("搜索焦点", self._receive_panel.focus_search, "Ctrl+F"))
    ...
    def _toggle_log_focus(self) -> None:
        ...
        label = "退出全屏日志模式" if is_focus else "全屏日志模式"
